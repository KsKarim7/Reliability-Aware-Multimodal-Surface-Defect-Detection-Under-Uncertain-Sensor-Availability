...
class PointTransformer(nn.Module):
    def __init__(self, group_size=128, num_group=1024, encoder_dims=384):
        ...
        scale = self.trans_dim ** -0.5
        self.proj = nn.Parameter(scale * torch.randn(self.trans_dim, self.output_dim))
    ...
    def forward(self, pts):
        ...
        # transformer
        x, feature_list = self.transformer(x, pos)
        feature_list = [self.norm(x)[:,1:].transpose(-1, -2).contiguous() for x in feature_list]
        features = torch.cat((feature_list[0],feature_list[1],feature_list[2]), dim=1) #1152

        x, tokens = x[:, 0], x[:, 1:].transpose(-1, -2)
        # print(x.shape, tokens.shape, features.shape, center.shape, ori_idx.shape, center_idx.shape)
        # torch.Size([1, 384]) torch.Size([1, 1024, 384]) torch.Size([1, 1152, 1024]), ...
        return x, tokens, features, center, ori_idx, center_idx 
        # else:
        #     B, C, N = pts.shape
        #     pts = pts.transpose(-1, -2)  # B N 3
        #     # divide the point cloud in the same form. This is important
        ...
        #     # transformer
        #     feature_list = self.transformer(x, pos)
