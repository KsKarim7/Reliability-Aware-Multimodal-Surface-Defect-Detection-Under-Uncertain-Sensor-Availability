...
class CustomResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        # as it will be added in the beginning, for both image and the depth branch
        self.prompt_length_half = prompt_length//3
        if idx == 0 and idx <= prompt_depth:
            ...
            self.prompts_dynamic_image = nn.init.normal_(torch.empty(self.prompt_length_half, 1, d_model, dtype=torch.float), std=0.02)
            self.prompts_dynamic_depth = nn.init.normal_(torch.empty(self.prompt_length_half, 1, d_model, dtype=torch.float), std=0.02)
    ...
    def forward(self, inputs, attn_mask: Optional[torch.Tensor] = None,):
        # For building feature gallery - step 0, such as inputs=x
        if not isinstance(inputs, list):
            # print("building feature gallery - step 0")
            x = inputs
            # print("input: ", x.shape)
            if isinstance(self.attn, Attention):
                x_res = self.attention(self.ln_1(x))
                ...
                x = x + self.ls_1(self.attention(q_x=self.ln_1(x), k_x=x, v_x=x, attn_mask=attn_mask))
                x = x + self.ls_2(self.mlp(self.ln_2(x)))
                # print("output: ", x.shape)
                return x
        # For building feature gallery - step i, such as inputs = [x, x_ori]
        elif isinstance(inputs, list) and len(inputs) == 2:
            # print("building feature gallery - step i")
            x, x_ori = inputs
        ...
        # For training
            ...
            # For training - step 0, such as inputs = [x, all_prompts, counter, missing_type]
            if not isinstance(inputs[0], list):
                        ...
                        if not (counter > len(compound_prompts_deeper) - 1):
                            ...
                            for i in range(len(missing_type)):
                                if missing_type[i]==0:  # modality complete
                                    ...
                                    prompts_dynamic.append(self.prompts_dynamic_depth)
                            ...
                            # print(prompts_staged_and_common.shape, prompts_dynamic.shape, visual_features.shape)
                ...
                if isinstance(self.attn, Attention):
                    ...
                    x = x + self.ls_2(self.mlp(self.ln_2(x)))
                    # print("output: ", x.shape)
                    return [x, compound_prompts_deeper, counter, None]
                
            # For training - step i, such as inputs = [[x, x_ori], all_prompts, counter, missing_type]
            else:
...
class Transformer(nn.Module):
    def __init__(
            ...
            act_layer: Callable = nn.GELU,
            norm_layer: Callable = LayerNorm,
            attention_layer: Callable = CustomResidualAttentionBlock,
            prompt_length: int = 36, 
            prompt_depth: int = 6,
    ):
        super().__init__()
        ...
        if attention_layer == CustomResidualAttentionBlock:
            self.resblocks = nn.ModuleList([
                attention_layer(
                    width, heads, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer, idx=idx, prompt_length=prompt_length, prompt_depth=prompt_depth)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            prompt_depth=prompt_length,
            prompt_length=prompt_depth
        )
...
class V2VTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            act_layer=act_layer,
            norm_layer=norm_layer,
            attention_layer=CustomResidualAttentionBlock,
    ...
    def forward(self, x: torch.Tensor, all_prompts: Optional[torch.Tensor] = None, missing_type: Optional[torch.Tensor] = None):
        ...
        if all_prompts is not None:
            # print(1)
            x = [x, all_prompts, 0, missing_type]
        ...
        # torch.Size([1, 640]) torch.Size([1, 225, 640]) torch.Size([1, 225, 896]) torch.Size([1, 225, 896])
        # print(pooled.shape, tokens.shape, self.mid_feature1.shape, self.mid_feature2.shape)
...
class TextTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            act_layer=act_layer,
            norm_layer=norm_layer,
            attention_layer=ResidualAttentionBlock,
        )
